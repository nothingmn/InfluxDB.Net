...
using InfluxDB.Core.Constants;
using InfluxDB.Core.Contracts;
using InfluxDB.Core.Enums;
using InfluxDB.Core.Helpers;
...
namespace InfluxDB.Core.Client
{
    ...
    {
        private const string UserAgent = "InfluxDB.Core";
        ...
        private async Task<InfluxDbApiResponse> RequestAsync(
            ...
            TimeSpan? requestTimeout = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (!headerIsBody)
            {
                responseContent = await ReadContentAsString(response);
            }
            else
            {
            ...
            bool includeAuthToQuery = true)
        {
            ...
            HttpRequestMessage request = await PrepareRequest(method, content, uri);

            Debug.WriteLine("[Request] {0}, instance id:{1}", request.ToJson(), client.GetHashCode());
            if (content != null)
            {
            ...
            try
            {
                //result = await client.SendAsync(request, completionOption, cancellationToken);  //this are broken..?
                result = client.SendAsync(request, completionOption, cancellationToken).Result;  //since that are broken, we hack
        ...
        private StringBuilder BuildUri(string path, Dictionary<string, string> extraParams, bool includeAuthToQuery)
        {
            ...
            {
                urlBuilder.AppendFormat("?{0}={1}&{2}={3}", QueryParams.Username, WebUtility.UrlEncode(_configuration.Username), QueryParams.Password, WebUtility.UrlEncode(_configuration.Password));
            }

            if (extraParams != null && extraParams.Count > 0)
            {
                var keyValues = new List<string>(extraParams.Count);
                keyValues.AddRange(extraParams.Select(param => String.Format("{0}={1}", param.Key, WebUtility.UrlEncode(param.Value))));
        ...
        private async Task<HttpRequestMessage> PrepareRequest(HttpMethod method, HttpContent content, StringBuilder urlBuilder)
        {
            ...
            if (content != null)
            {
                request.Content = new ByteArrayContent(await content.ReadAsByteArrayAsync());
            }
